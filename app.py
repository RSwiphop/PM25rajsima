...
# --- แก้ไข: ใช้ st.session_state เพื่อจัดการสถานะของ time_input ---
# ตรวจสอบว่ามีค่า time_input ใน session_state หรือไม่ ถ้าไม่มีให้กำหนดค่าเริ่มต้น
if 'time_input' not in st.session_state:
    st.session_state.time_input = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

# ฟังก์ชันสำหรับอัปเดต session_state เมื่อมีการเปลี่ยนค่าใน text_input
def update_time_state():
    st.session_state.time_input = st.session_state.time_key
...
with col2:
    try:
        ...
        st.warning("⚠️ ไม่พบไฟล์ 'images/logo.png' กรุณาตรวจสอบโฟลเดอร์บน GitHub.")
...
# --- แก้ไข: ใช้ st.session_state และ key ใน st.text_input เพื่อเก็บค่าที่ผู้ใช้ป้อน ---
st.text_input(
    "วันเวลา (YYYY-MM-DD HH:MM:SS)",
    value=st.session_state.time_input,
    key="time_key", # ใช้ key เพื่อเชื่อมกับ session_state
    on_change=update_time_state, # เมื่อค่าเปลี่ยน ให้เรียกฟังก์ชันเพื่ออัปเดต session_state
...
if st.button("ทำนายค่า PM2.5", use_container_width=True, type="primary"):
    with st.spinner("⏳ กำลังทำนาย..."):
        try:
            # ดึงค่าเวลาจาก session_state ที่ถูกต้อง
            time_input = st.session_state.time_input
...
try:
    ...
    st.warning("⚠️ ไม่พบไฟล์ 'images/AI.png' กรุณาตรวจสอบโฟลเดอร์บน GitHub.")
